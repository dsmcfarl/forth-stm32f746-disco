...
align
#6 constant font-char-bytes-4x6
...
align
#18 constant font-char-bytes-8x16

0 variable font-x   0 variable font-y
0 variable font
0 variable font-char-bytes
0 variable drawcharacterbitmap-addr


#4 constant font-width-4x6
#6 constant font-height-4x6
#1 #31 lshift constant font-bit-test-mask-4x6
: drawbitpattern-4x6 ( x -- x-shifted-font-width )
  font-width-4x6 0 do
    dup
    font-bit-test-mask-4x6 and
    if font-x @ font-y @ putpixel then shl 1 font-x +!
  loop
  font-width-4x6 negate font-x +!
;

: drawcharacterbitmap-4x6 ( c-addr -- )
  dup h@ swap 2+ h@ 16 lshift or   \ Fetch 32 bits on an 2-aligned location
  font-height-4x6 0 do
    drawbitpattern-4x6 1 font-y +!
    loop
  drop
  font-height-4x6 negate
  font-y +!
  font-width-4x6
  font-x +!
;

#8 constant font-width-8x16
#16 constant font-height-8x16
#1 #7 lshift constant font-bit-test-mask-8x16
: drawbitpattern-8x16 ( x -- x-shifted-font-width )
  font-width-8x16 0 do
    dup
    font-bit-test-mask-8x16 and
    if font-x @ font-y @ putpixel then shl 1 font-x +!
  loop
  font-width-8x16 negate font-x +!
;

: drawcharacterbitmap-8x16 ( c-addr -- )
  font-height-8x16 0 do
    dup c@
    drawbitpattern-8x16 drop 1 font-y +!
    1+
    loop
  drop
  font-height-8x16 negate
  font-y +!
  font-width-8x16
  font-x +!
;
...
: unicode>bitmap ( x -- c-addr ) \ Translates Unicode character to address of 8x8 bitmap.
  ...
  dup h@ while \ As long as there are more characters left in the glyph collection...
    font-char-bytes @ + \ Not this one, skip it and its bitmap
  repeat
...
\ -------------------------------------------------------------
\  Write an Unicode character with bitmap font
\ -------------------------------------------------------------




: drawunicode ( x -- ) unicode>bitmap drawcharacterbitmap-addr @ execute ;
